...
from apps.legal.constants import (
    LEGAL_DOCUMENTS,
    PRIVACY_VERSION,
    REQUIRED_DOCUMENTS,
    TERMS_VERSION,
...
from apps.legal.selectors.acceptance_selectors import get_pending_documents, has_accepted
...
class VersionBumpTests(TestCase):
    ...
    def test_a_bump_makes_only_that_document_pending_again(self):
        with patch(
            "apps.legal.constants.LEGAL_DOCUMENTS", bumped_registry(terms="2027-01-01")
        ):
    ...
    def test_accepting_the_new_version_keeps_the_old_row(self):
        ...
        with patch("apps.legal.constants.LEGAL_DOCUMENTS", bumped):
            record_acceptance(user=self.user, documents=["terms"])
    ...
    def test_a_rolled_back_version_puts_the_user_back_in_the_gate(self):
        ...
        with patch(
            "apps.legal.constants.LEGAL_DOCUMENTS", bumped_registry(terms="2026-01-01")
        ):
            self.assertEqual(get_pending_documents(self.user), ["terms"])
