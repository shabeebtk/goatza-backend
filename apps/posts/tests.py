...
# =====================================================================
# REMOVED: PostMediaDimensionsTests + VideoEagerDerivativeTests
#
# Both covered provider behaviour that no longer exists — width/height read
# back off the storage provider, and eager video-derivative generation. The
# client now measures dimensions and encodes video before upload.
#
# Two assertions in the deleted set were NOT about the provider and still
# describe live behaviour, so they want rewriting against the current flow:
#   - a list response exposes width/height
#   - legacy media with no dimensions serializes them as null
# =====================================================================
...
class PostHashtagTests(APITestCase):
    ...
    def test_extract_hashtags_caps_and_truncates(self):
        from apps.posts.services.post_content_service import (
...
class PostMentionTests(APITestCase):
    ...
    def test_grouped_text_renders_for_mention(self):
        from apps.notifications.services.grouping_service import (
